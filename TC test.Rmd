...
In this section we will be implementing function similar to `costFunctionReg.m`. That is the function will have two outputs: cost and gradient. In place of *Octave/Matlab*'s `fminunc` we will be using *R*'s general purpose optimization function `optim`. The format of two functions is remarkably similar.  
